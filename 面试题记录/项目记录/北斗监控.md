...
##### 白屏判断时机
```bash
1、首先提供了自己上报已渲染首屏的api，可业务方自己主动上报
2、若业务方未主动上报，则定时调用 document.elementfrompoint(x,y) api，
  来获取(x,y)位置的元素，与后台配置的对应位置元素对比，匹配度达到4/8则认为已完成首屏渲染
3、若超过5s，扔未得到首屏渲染成功状态，则认为页面白屏了
# 没有用domLoad事件是因为页面基本都有异步情况，用此方法就不准确了。
```
