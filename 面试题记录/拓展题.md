...
```
表现：
  ios: 输入框获取焦点，键盘弹起，页面高度没有改变，只是页面整体往上滚动了软键盘的高度；收起键盘，输入框失去焦点。
  (但是收起的时候，往上滚动的距离并不会滚下来，可能会有滚动超出页面高度的情况，导致页面下边留白的问题。)
  android: 输入框获取焦点，键盘弹起，页面高度变成原可视区高度减去键盘高度；点输入框外区域收起键盘时，输入框失去焦点，点软键盘上的收起，输入框不会失去焦点。
  (由于弹起键盘的时候，页面没有滚动，所以会有fix定位被覆盖，输入框没有在可视区的问题。)
解决方案：
  ios监控输入框获取焦点和失去焦点事件，做对应处理
  Android：检测窗口变化事件，做定位，页面滚动等对应处理
```
...
defer: 立即下载，延迟执行，会按defer文件的**顺序执行**
async：立即下载，异步执行，但是脚本**无顺序**
##### 重绘和回流
...
-------------------
#### 单点登录 SSO
场景：一个系统有多个域名不同的子系统，流程是：用户浏览器访问系统a，跳转到sso系统，登录成功后返回系统a；此时打开系统b，系统b也是已登录状态。(系统ab可以主域名不同);
<img src="./imgs/CAS.png" width=300/>
```
一、用户访问系统A
  1. 需登录时跳转到SSO系统
  2. SSO系统账号密码登录
  3. SSO的服务器存储session，生成一个 sessionId 传给 SSO 系统的浏览器端
  4. 浏览器写入SSO域下的cookie，并生一个ST
  5. 将ST传入系统A
  6. 系统A 用接收到的ST请求SSO系统做校验
  7. 校验成功后，系统A的服务器将登录态写入 session，并种A系统域名下的登录cookie
二、用户访问系统B
  1. 需登录时跳转到SSO系统
  2. SSO已登录，则直接生成一个ST，传入系统B
  3. 系统B用此ST请求SSO系统做校验，
  4. 校验成功后，系统B的服务器将登录态写入 session，并设置B系统域名下的登录cookie
  看起来就是系统B不需要再走登录流程
```
#### pc端二维码登录实现
...
解决方案: 
1、接口默认只展示一年内的订单，即可以避免这种问题，又可以加快速度；一年以上订单只可以搜索展示
2、前端兼容异常数据
```
* 用户设备缓存的数据影响
```
